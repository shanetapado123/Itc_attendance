...
if (isset($_POST['payroll'])) {
  header('location: payrollworked.php');
}?>

<?php
if (isset($_POST['payrolluser'])) {
  header('location: payrolluser.php');
}?>
   ...
   <button type="submit" name='cancel' class=" btn btn-danger mb-3" style="float: right;">Logout</button>
    ...
    <div class="col">
      <div class="card text-center">
        ...
        <div class="card-body">
          <a href="payrollworked.php"> <img src="images/payroll.png" alt="login image" width="25%" height="25%"></a>
          <p class="card-text text-muted">Dashboard</p>
          <button type="payroll" name='payroll' class=" btn btn-success mb-3" style="float: left;">Employee</button>
          <button type="payrolluser" name='payrolluser' class=" btn btn-secondary mb-3" style="float: right;">User</button>
        </div>
      </div>
    </div>
